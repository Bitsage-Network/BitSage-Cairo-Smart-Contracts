...
# SAGE Network Event Testing Script
...
# Keystore
KEYSTORE="../SAGE_Network_Backup/20250711_061352/testnet_keystore.json"
TEST_KEYSTORE="temp_account.json"
NETWORK="sepolia"

echo -e "${BLUE}🎯 SAGE Network Event Testing${NC}"
echo -e "${BLUE}=============================${NC}"
echo ""
...
if echo "test" | starkli account deploy ../SAGE_Network_Backup/20250711_061352/internal_docs/testnet_deployer.json --keystore "$KEYSTORE" --network "$NETWORK" 2>&1; then
...
echo ""
echo -e "${BLUE}Phase 2: SAGE Token Events${NC}"
echo ""

# Test 1: Approve tokens (generates Approval event)
test_invoke "$SAGE_TOKEN" "approve" "$TEST_ACCOUNT 0x3e8" "../SAGE_Network_Backup/20250711_061352/internal_docs/testnet_deployer.json" "$KEYSTORE" "Approve 1000 tokens to test account"

# Test 2: Transfer tokens (generates Transfer event)
test_invoke "$SAGE_TOKEN" "transfer" "$TEST_ACCOUNT 0x64" "../SAGE_Network_Backup/20250711_061352/internal_docs/testnet_deployer.json" "$KEYSTORE" "Transfer 100 tokens to test account"
...
test_invoke "$TREASURY_TIMELOCK" "propose_transaction" "$SAGE_TOKEN 0x0 0x0 0x746573745f70726f706f73616c" "$TEST_KEYSTORE" "$KEYSTORE" "Propose test transaction"
...
echo "1. Approval event (SAGE Token)"
echo "2. Transfer event (SAGE Token)"  
