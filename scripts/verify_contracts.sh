...
# SAGE Network Contract Verification Script
set -e
...
echo -e "${BLUE}🔍 SAGE Network Contract Verification${NC}"
echo -e "${BLUE}======================================${NC}"
...
echo -e "${YELLOW}📋 Testing SAGE Token Contract${NC}"
test_call "$SAGE_TOKEN" "name" "" "Token Name"
test_call "$SAGE_TOKEN" "symbol" "" "Token Symbol"
test_call "$SAGE_TOKEN" "decimals" "" "Token Decimals"
test_call "$SAGE_TOKEN" "total_supply" "" "Total Supply"
test_call "$SAGE_TOKEN" "balance_of" "$DEPLOYER_ADDR" "Deployer Balance"
test_call "$SAGE_TOKEN" "balance_of" "$TEST_ACCOUNT" "Test Account Balance"
